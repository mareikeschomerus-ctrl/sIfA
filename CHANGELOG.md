...
- **Tool / Service field now wraps.** It was a single-line input in a narrow column, so anything longer than a few words (e.g. a list of AI tools) scrolled sideways inside a tiny box instead of being visible while editing. It's now a multiline field that auto-grows, matching how "How was AI used?" already behaves, and the column itself is wider (160px → 210px). The native browser autocomplete this field used (which can't attach to a multiline field) is replaced with the same one-click "↩ suggestion" chips already used for contributor names, built from every distinct tool name typed anywhere in the table.
- **Contributor level defaults to something sensible instead of always "Equal".** Adding the first contributor to a role now defaults their level to **Sole** (there's no one else to be equal to yet). Adding a second contributor flips both to **Equal**, a reasonable starting assumption for two people sharing a role, adjustable by hand afterward (e.g. to Lead/Support if the split isn't actually even). A third or later contributor still defaults to Equal, consistent with the tool's existing rule that all contributors must share Equal once a role is in that mode.
- **Project name now shows on the figure and table, not just the toolbar.** When a Project name is set, it appears as a subtitle under "sIfA" everywhere the figure or table gets generated: the live "View figure" panel, the Figure-only SVG/JPEG export, the composite Table & Figure SVG/JPEG export, and both PDF paths (the toolbar's "Save as PDF," which includes the figure, and the plain table-only print via ⌘/Ctrl+P). Omitted entirely when no project name is set — no blank line left behind.
- **Save now shows which file it's about to overwrite, and a new "Save As…" lets you deliberately redirect it.** Once Save remembers a file (see the save-in-place change above), that choice was previously invisible — a single wrong click in the "Save as…" dialog would silently overwrite the wrong file on every subsequent save, with no indication anything was off. The 💾 Save button's tooltip and the Save dropdown now both show the exact filename Save will write to (or "Not yet saved — will ask where" before the first save). A new **💾 Save As… (.json)** action always opens a fresh picker regardless of what Save currently remembers, for deliberately switching files or recovering from a wrong pick.
